    ...
    <el-col :span="24" class="header">
      ...
      <div class="userInfo">
		...
		<el-col :span="24" class="main">
			<div v-bind:class="[isFolded ? 'hidden' :'sidebar']" id="sidebarDiv">
				<div class="info" id="infoDiv">
					<div class="myProjectLogo">
						<label style="cursor:pointer" @click="queryProjectByKeyWord('')">我的项目</label>
						...
						>
							<div style>
								<div style="float:left;color:#ababab">排序方式</div>
				...
				<div class="searchBlock" id="searchDiv">
					<div style="width:250px;margin:0 auto;">
						<el-input
							...
							@keyup.enter.native="queryProjectByKeyWord(searchKeyWord)"
						></el-input>
						<i class="fa fa-sort-amount-asc fa-fw"></i>
					</div>
				</div>
				...
				<div v-for="(project,index) in projects" @click="handleProjectBoxClick(project,index)">
					...
					<div :class="index==0?'projectBox clickedBox':'projectBox'" :id="'box'+index">
						<div class="btnGroup">
							...
							<span @click.stop="showUpdateProject(project)" title="修改">
								<i class="fa fa-pencil fa-fw hidden"></i>
							</span>
							<span @click.stop="deleteProject(project)" title="删除">
								<i class="fa fa-trash-o fa-fw hidden"></i>
							</span>
						</div>
						<div class="projectName">{{project.name}}</div>
						<div class="projectDate">{{project.date}}</div>
						<div class="projectDescription">{{project.description}}</div>
					</div>
				</div>
				<div v-show="projects.length === 0" class="emptyProject">暂无数据</div>
			</div>
			<div
				v-bind:class="[isFolded ? 'hidden' :'location']"
			>当前位置 {{this.currentBoxIndex+1}}/{{this.projects.length}}</div>
		...
		<el-dialog
			...
			:visible.sync="updateProjectForm.visible"
			:width="formAttr.dialogWidth"
		>
			<el-form :model="updateProjectForm" :rules="updateProjectRules" ref="updateProjectForm">
				...
				<el-form-item label="项目描述" :label-width="formAttr.formLabelWidth" prop="projectDescription">
					<el-input
						type="textarea"
						:rows="4"
						v-model="updateProjectForm.projectDescription"
						auto-complete="off"
					></el-input>
				</el-form-item>
			</el-form>
		...
		<el-dialog title="设置" :visible.sync="accountCogForm.visible" width="35%">
			<el-tabs
				v-model="accountCogForm.activeName"
				:tab-position="accountCogForm.tabPosition"
				style="height:auto;"
			>
	...
	export default {
		...
		methods: {
					...
					if (data.code === 1) {
						...
						if (projectCount != 0) {
							...
							if (currentRoute === "主页") {
								this.$router.push({
									name: "原始日志",
									params: { id: 1, project: this.projects[0] }
								});
								this.currentProject = this.projects[0];
					...
					.then(() => {
						...
						deleteProject(params).then(data => {
							if (data.code === 1) {
								...
								//如果删除的是最后一个
								if (
									deletedProjectIndex === length - 1 &&
									this.projectCount != 0
								) {
									this.lastClickedBox = "";
									this.handleProjectBoxClick(
										this.projects[this.projectCount - 1],
										this.projectCount - 1
									);
								}
							}
			...
			addProject() {
				this.$refs.addProjectForm.validate(valid => {
					if (valid) {
						...
						let projectDescription = this.addProjectForm
							.projectDescription;
						let userId = this.uid;
						let params = {
			...
			updateProject() {
				this.$refs.updateProjectForm.validate(valid => {
					if (valid) {
						...
						let projectDescription = this.updateProjectForm
							.projectDescription;
			...
			handleProjectBoxClick(project, index) {
				...
				let id = "box" + index;
				//重复按下
				if (id === this.lastClickedBox) return;
				//clickedBox前面必须加空格
				document.getElementById(id).className += " clickedBox";
				if (this.lastClickedBox !== "")
					document
						.getElementById(this.lastClickedBox)
						.setAttribute("class", "projectBox");
